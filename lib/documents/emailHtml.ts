...
export function buildEmailHeaderHtml(opts: {
  ...
  const secondary = opts.secondaryDateLabel
    ? `<p style="font-family:monospace;font-size:11px;color:var(--brand-text-secondary);margin:0;">${escHtml(opts.secondaryDateLabel)}: ${escHtml(opts.secondaryDate ?? '—')}</p>`
    : ''
  return `
      <div style="background:${BUSINESS_INK};padding:28px 32px;border-bottom:4px solid ${BUSINESS_ACCENT};">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;">
          <div>
            <p style="font-family:monospace;font-size:14px;font-weight:700;color:var(--brand-surface);margin:0 0 6px;letter-spacing:0.05em;">${escHtml(BUSINESS.brandFull)}</p>
            <p style="font-family:monospace;font-size:11px;color:var(--brand-text-secondary);margin:0 0 2px;">${escHtml(BUSINESS.legalName)}</p>
            <p style="font-family:monospace;font-size:11px;color:var(--brand-text-secondary);margin:0 0 2px;">${escHtml(BUSINESS.addressLine1)}, ${escHtml(BUSINESS.addressLine2)}</p>
            <p style="font-family:monospace;font-size:11px;color:var(--brand-text-secondary);margin:0 0 2px;">${escHtml(BUSINESS.email)}</p>
            <p style="font-family:monospace;font-size:11px;color:var(--brand-text-secondary);margin:0;">${escHtml(BUSINESS.phone)}</p>
          </div>
          <div style="text-align:right;">
            <p style="font-family:monospace;font-size:18px;font-weight:700;color:var(--brand-text-inverted);margin:0 0 6px;letter-spacing:0.1em;">${escHtml(opts.docTypeLabel)}</p>
            ...
            <p style="font-family:monospace;font-size:11px;color:var(--brand-text-secondary);margin:0 0 2px;">${escHtml(opts.dateLabel)}: ${escHtml(opts.date)}</p>
            ${secondary}
          </div>
        </div>
      </div>`
...
export function buildEmailAddressBlockHtml(opts: { nameOnly: string; company: string; address?: string | null; language?: DocumentLanguage | null }): string {
  ...
  const lines = [
    `<p style="color:var(--brand-text);font-size:15px;margin:0 0 2px;font-weight:600;">${escHtml(opts.nameOnly)}</p>`,
    opts.company ? `<p style="color:var(--brand-text-muted);font-size:13px;margin:0 0 2px;">${escHtml(opts.company)}</p>` : '',
    opts.address ? `<p style="color:var(--brand-text-muted);font-size:13px;margin:0;">${escHtml(opts.address)}</p>` : '',
  ].filter(Boolean).join('')

  return `
      <div style="padding:20px 32px;background:${BRAND_COLORS.surfaceSubtle};border-bottom:1px solid var(--brand-border);">
        <p style="color:var(--brand-text-secondary);font-size:10px;margin:0 0 8px;font-family:monospace;text-transform:uppercase;letter-spacing:0.12em;">${escHtml(t.to)}</p>
        ${lines}
      </div>`
}
...
export function buildEmailTableHeaderHtml(language?: DocumentLanguage | null): string {
  ...
  return `
          <tr style="background:${BRAND_COLORS.surfaceSubtle};">
            <th style="padding:8px 10px;font-family:monospace;font-size:10px;color:var(--brand-text-secondary);text-transform:uppercase;letter-spacing:0.08em;text-align:left;">${escHtml(t.colPos)}</th>
            <th style="padding:8px 10px;font-family:monospace;font-size:10px;color:var(--brand-text-secondary);text-transform:uppercase;letter-spacing:0.08em;text-align:left;">${escHtml(t.colDescription)}</th>
            <th style="padding:8px 10px;font-family:monospace;font-size:10px;color:var(--brand-text-secondary);text-transform:uppercase;letter-spacing:0.08em;text-align:right;">${escHtml(t.colQuantity)}</th>
            <th style="padding:8px 10px;font-family:monospace;font-size:10px;color:var(--brand-text-secondary);text-transform:uppercase;letter-spacing:0.08em;text-align:right;">${escHtml(t.colUnitPrice)}</th>
            <th style="padding:8px 10px;font-family:monospace;font-size:10px;color:var(--brand-text-secondary);text-transform:uppercase;letter-spacing:0.08em;text-align:right;">${escHtml(t.colAmount)}</th>
          </tr>`
}
...
export function buildEmailBankBlockHtml(reference: string, language?: DocumentLanguage | null): string {
  ...
  return `
        <div style="background:${BRAND_COLORS.surfaceSubtle};border-left:3px solid ${BUSINESS_ACCENT};padding:16px 20px;margin-bottom:28px;">
          ...
          <p style="font-family:monospace;font-size:13px;color:var(--brand-surface-subtle);margin:0 0 3px;">${escHtml(t.accountHolder)}: ${escHtml(BANK_TRANSFER.beneficiary)}</p>
          <p style="font-family:monospace;font-size:13px;color:var(--brand-surface-subtle);margin:0 0 3px;">IBAN: ${escHtml(BANK_TRANSFER.iban)}</p>
          <p style="font-family:monospace;font-size:13px;color:var(--brand-surface-subtle);margin:0 0 3px;">BIC: ${escHtml(BANK_TRANSFER.bic)}</p>
          <p style="font-family:monospace;font-size:13px;color:var(--brand-surface-subtle);margin:0;">${escHtml(t.paymentReference)}: ${escHtml(reference)}</p>
        </div>`
}
...
export function buildEmailVatClauseHtml(language?: DocumentLanguage | null, extra?: string): string {
  ...
  return `<p style="font-family:monospace;font-size:11px;color:var(--brand-text-secondary);margin:12px 0 20px;">${escHtml(clause)}${suffix}</p>`
}
