...
| 33 | **The same case study is quoted in two currencies** | The homepage proof strip says `€14k/mo` — `14.000 €/Monat` in German. The case-studies page says `£14,000/month` — `14.000 £/Monat` in German. The same claimed client saving, in euros on one page and pounds sterling on another, in both languages, on a site selling to German SMEs from Essen. **Not corrected here.** Either figure could be the true one and changing a stated client saving is a claim, not a formatting fix. It needs one answer from Marcel: which currency, and then both pages say it. |
