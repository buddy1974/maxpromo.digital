...
export default function LeadsPage() {
  ...
  const inpStyle: React.CSSProperties = { width: '100%', background: 'var(--brand-surface)', border: '1px solid var(--brand-border)', borderRadius: '4px', color: 'var(--brand-text)', fontFamily: sans, fontSize: '13px', padding: '9px 12px', outline: 'none', boxSizing: 'border-box' }
  ...
  return (
    <div style={{ padding: '32px 40px' }}>
      ...
      {showNew && (
        <div style={{ position: 'fixed', inset: 0, background: 'color-mix(in srgb, var(--brand-text) 45%, transparent)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
          <div style={{ background: 'var(--brand-surface-subtle)', border: '1px solid color-mix(in srgb, var(--brand-primary) 30%, transparent)', width: '100%', maxWidth: '440px', borderRadius: '4px', padding: '28px' }}>
            <p style={{ fontFamily: mono, fontSize: '10px', color: 'var(--brand-primary-text)', letterSpacing: '0.2em', textTransform: 'uppercase', margin: '0 0 20px' }}>{t.leads.newLeadModal}</p>
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
              ...
              <button type="button" onClick={() => { setShowNew(false); setCreateErr('') }} style={{ background: 'none', border: '1px solid var(--brand-border)', borderRadius: '4px', color: 'var(--brand-text)', fontFamily: sans, fontSize: '13px', padding: '10px 16px', cursor: 'pointer' }}>
                {t.common.cancel}
              </button>
            </div>
          </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontFamily: grotesk, fontSize: '24px', fontWeight: 700, color: 'var(--brand-text)', letterSpacing: '-0.02em', margin: '0 0 4px' }}>{t.leads.heading}</h1>
        ...
        <button onClick={() => { setShowNew(true); setCreateErr('') }} style={{ background: 'var(--brand-primary)', border: 'none', borderRadius: '4px', color: 'var(--brand-text)', fontFamily: sans, fontWeight: 700, fontSize: '13px', padding: '10px 18px', cursor: 'pointer' }}>
          {t.leads.newLead}
        </button>
      </div>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', alignItems: 'center', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', gap: '4px' }}>
          {['all', ...STATUSES].map(key => (
            <button key={key} onClick={() => setTab(key)} style={{ fontFamily: mono, fontSize: '10px', letterSpacing: '0.1em', textTransform: 'uppercase', padding: '7px 12px', border: 'none', cursor: 'pointer', background: tab === key ? 'var(--brand-primary)' : 'transparent', color: tab === key ? 'var(--brand-text)' : 'var(--brand-text-muted)' }}>
              {key === 'all' ? t.status.filterAll : statusLabel(key)}
            </button>
          ))}
        </div>
        <input placeholder={t.common.searchPlaceholder} value={search} onChange={e => setSearch(e.target.value)} style={{ background: 'var(--brand-surface-subtle)', border: '1px solid var(--brand-border)', color: 'var(--brand-text)', fontFamily: sans, fontSize: '13px', padding: '7px 14px', outline: 'none', width: '220px' }} />
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} style={{ padding: '24px 16px', fontFamily: sans, fontSize: '13px', color: 'var(--brand-text-muted)' }}>{t.leads.empty}</td></tr>
            ) : (
              filtered.map(lead => (
                <tr key={lead.id} style={{ borderBottom: '1px solid var(--brand-border)', cursor: 'pointer' }} onClick={() => setSelected(lead)}>
                  <td style={{ padding: '12px 16px' }}>
                    <p style={{ fontFamily: sans, fontSize: '13px', color: 'var(--brand-text)', margin: '0 0 2px' }}>{lead.name || t.common.notAvailable}</p>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }} onClick={e => e.stopPropagation()}>
                      {lead.status !== 'contacted' && (
                        <button onClick={() => updateStatus(lead.id, 'contacted')} style={{ fontFamily: mono, fontSize: '10px', color: 'var(--semantic-info)', background: 'none', border: 'none', cursor: 'pointer', padding: 0, letterSpacing: '0.04em' }}>
                          {t.leads.actionContacted}
                        </button>
                      )}
                      {lead.status !== 'converted' && (
                        <button onClick={() => updateStatus(lead.id, 'converted')} style={{ fontFamily: mono, fontSize: '10px', color: 'var(--semantic-success)', background: 'none', border: 'none', cursor: 'pointer', padding: 0, letterSpacing: '0.04em' }}>
                          {t.leads.actionConvert}
                        </button>
                      )}
                      <button onClick={() => deleteLead(lead.id)} style={{ fontFamily: mono, fontSize: '10px', color: 'var(--brand-text-muted)', background: 'none', border: 'none', cursor: 'pointer', padding: 0, letterSpacing: '0.04em' }}>
      ...
      {selected && (
        <div style={{ position: 'fixed', top: 0, right: 0, width: '400px', height: '100vh', background: 'var(--brand-surface)', borderLeft: '1px solid var(--brand-border)', zIndex: 200, overflowY: 'auto' }}>
          ...
          <div style={{ padding: '24px' }}>
            <h2 style={{ fontFamily: grotesk, fontSize: '18px', fontWeight: 700, color: 'var(--brand-text)', margin: '0 0 4px' }}>{selected.name || t.leads.anonymous}</h2>
            {selected.company && <p style={{ fontFamily: mono, fontSize: '11px', color: 'var(--brand-text-muted)', margin: '0 0 16px' }}>{selected.company}</p>}
