...
const APPROACH_KEYS = ['approach1', 'approach2', 'approach3', 'approach4'] as const
const UC_KEYS       = ['uc1', 'uc2', 'uc3', 'uc4', 'uc5', 'uc6'] as const
const UC_ICONS      = ['→', '◰', '⊟', '▤', '◇', '⌗'] as const
  ...
  return (
    <main style={{ background: 'hsl(240 14% 4%)' }}>
          ...
          <div style={{ display: 'grid', gap: '1px', background: 'hsl(240 10% 16%)', borderRadius: '12px', overflow: 'hidden' }} className="grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            {APPROACH_KEYS.map((k, i) => (
              <div key={k} style={{ background: 'hsl(240 12% 7%)', padding: '2.5rem 2rem' }}>
                <p style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: '48px', lineHeight: 1, marginBottom: '1rem', color: '#F97316' }}>
          ...
          <div style={{ display: 'grid', gap: '1px', background: 'hsl(240 10% 16%)', borderRadius: '16px', overflow: 'hidden' }} className="grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            {UC_KEYS.map((k, i) => (
              <div key={k} style={{ background: 'hsl(240 12% 7%)', padding: '2.5rem', position: 'relative', overflow: 'hidden' }}>
                ...
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '20px', color: '#F97316', display: 'block', marginBottom: '16px' }}>{UC_ICONS[i]}</span>
      ...
      <section style={{ background: 'hsl(240 14% 4%)', padding: '4rem 2rem', borderBottom: '1px solid hsl(40 30% 96% / 0.06)' }}>
        <div style={{ maxWidth: '56rem', margin: '0 auto' }}>
          <p style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: '#F97316', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: '1.5rem' }}>
            {t('notThisEyebrow')}
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {notThisItems.map((item, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', background: 'hsl(240 12% 7%)', border: '1px solid hsl(40 30% 96% / 0.06)', padding: '16px 20px' }}>
                <span style={{ color: 'hsl(0 84% 60% / 0.7)', fontFamily: 'var(--font-mono)', fontSize: '14px', flexShrink: 0 }}>✕</span>
