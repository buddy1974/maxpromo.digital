...
export default async function SocialMediaPage() {
  ...
  return (
    <main style={{ background: 'var(--color-bg)' }}>
      ...
      {/* Service image — degrades gracefully to a placeholder if the asset is missing */}
      <div style={{ background: 'var(--color-bg)', padding: '0 2rem 3.5rem' }}>
        <div style={{ maxWidth: '56rem', margin: '0 auto' }}>
          <ServiceImage
            src="/images/services/social-media/hero.jpg"
            alt="Content calendar and social media workflow on a business desk"
            placeholder="image: phone · content calendar · business desk"
          />
        </div>
      </div>
        ...
        <div style={{ maxWidth: '64rem', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gap: '1px', background: 'var(--color-border)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-card)', overflow: 'hidden' }} className="grid-cols-1 md:grid-cols-2">
            <div style={{ background: 'var(--color-bg-section)', padding: '2rem 2.5rem' }}>
              <p style={{ fontFamily: 'var(--brand-font-sans)', fontSize: '11px', color: 'var(--color-text-secondary)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '1rem' }}>
                {t('proofBeforeLabel')}
              </p>
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {proofBefore.map((item, i) => (
                  <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', fontFamily: 'var(--font-body)', fontSize: '15px', color: 'var(--color-text-secondary)' }}>
                    <span style={{ color: 'var(--color-text-secondary)', flexShrink: 0, fontFamily: 'var(--font-mono)' }}>✕</span>
                    {item}
                  </li>
                ))}
              </ul>
            ...
            <div style={{ background: 'var(--color-bg)', padding: '2rem 2.5rem' }}>
              <p style={{ fontFamily: 'var(--brand-font-sans)', fontSize: '11px', color: 'var(--brand-text-secondary)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '1rem' }}>
                {t('proofAfterLabel')}
              </p>
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {proofAfter.map((item, i) => (
                  <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', fontFamily: 'var(--font-body)', fontSize: '15px', color: 'var(--color-text-primary)' }}>
                    <span style={{ color: 'var(--brand-text-secondary)', flexShrink: 0, fontFamily: 'var(--font-mono)' }}>✓</span>
                    {item}
                  </li>
                ))}
              </ul>
