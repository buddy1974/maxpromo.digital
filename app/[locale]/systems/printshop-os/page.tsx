 ...
 * 2026-07-26. Retires the previous ~39KB hand-authored page.tsx (plus its
 * PrintshopContactForm.tsx, now unreferenced — see the consolidation
 * report for the site-wide contact-form bug that component shared with
 * six siblings, and the fabricated real-time-pricing/auto-invoicing/FR-
 * locale claims the retired page made that the registry's corrected
 * entry does not). Renders the same registry-driven LandingEngine used
 * on the branded external domain (smartprintshop.de), here under the
 * maxpromo.digital hub layout (Navbar/Footer/CookieBanner via
 * app/[locale]/layout.tsx — x-mp-mode is 'hub' on this host, not
 * 'showcase').
 *
 * The registry (lib/registry/products.ts, PRINTSHOP_OS entry) is the
 * only source of this page's public copy — do not hardcode content here.
 */
const REGISTRY_SLUG = 'printshop-os'
const CANONICAL_PATH = '/systems/printshop-os'
...
export default async function PrintShopOSPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
