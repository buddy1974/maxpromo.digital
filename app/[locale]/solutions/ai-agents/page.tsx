        ...
        <div style={{ maxWidth: '56rem', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {notThisItems.map((item, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', background: 'var(--color-bg-section)', border: '1px solid var(--color-border)', padding: '16px 20px', borderRadius: 'var(--radius-card)' }}>
                <span style={{ color: 'var(--color-text-secondary)', fontFamily: 'var(--font-mono)', fontSize: '14px', flexShrink: 0 }}>✕</span>
                <p style={{ fontFamily: 'var(--font-body)', fontSize: '16px', color: 'var(--color-text-secondary)', lineHeight: 1.6, margin: 0 }}>{item}</p>
              </div>
            ))}
          </div>
        </div>
