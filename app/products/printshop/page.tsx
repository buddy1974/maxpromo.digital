...
function PrintshopContactForm() {
  ...
  return (
    ...
    >
      ...
      >
        {status === 'loading' ? 'Sending...' : status === 'success' ? '✓ Sent' : 'GET PRINTSHOP OS →'}
      </button>
      {status === 'success' && (
...
const FEATURES = [
  { icon: '[ SHOP ]',   name: 'Live Product Catalogue',   desc: 'Fixed size and custom per-m² pricing. Customers configure quantity and size — price calculates live. No quote request needed.' },
  { icon: '[ AI ]',     name: 'AI Prepress Check',        desc: 'Customer uploads a file. AI checks DPI, bleed, dimensions, and colour profile instantly. Wrong file caught before it reaches production.' },
  { icon: '[ EDIT ]',   name: 'Online Design Editor',     desc: 'Fabric.js canvas built in. Customers add text, shapes, and logos directly in the browser. Design saved to their order.' },
  { icon: '[ PROD ]',   name: 'Production Queue',         desc: 'Every paid order enters the queue automatically. Move jobs from Queued → In Progress → Done. Print sheet generated per order.' },
  { icon: '[ STRIPE ]', name: 'Stripe Payments Built In', desc: 'Full checkout flow with tax calculation, multi-item cart, and invoice PDF download. Test mode included for demos.' },
  { icon: '[ i18n ]',   name: 'Three Languages',          desc: 'English, German, French — full platform translation including shop, checkout, admin, and all email templates.' },
]
        ...
        <section style={{ padding: '5rem 2rem', borderBottom: '1px solid #1A1A1A' }}>
          <div style={{ maxWidth: '72rem', margin: '0 auto' }}>
            ...
            <h1 style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 'clamp(2.5rem, 5vw, 4rem)', letterSpacing: '-0.04em', color: '#F0F0F0', lineHeight: 1.1, marginBottom: '1.5rem', whiteSpace: 'pre-line' }}>
              {'A complete print shop.\nRunning itself online.'}
            </h1>
            <p style={{ fontFamily: 'var(--font-body)', fontSize: '18px', color: '#666666', maxWidth: '600px', lineHeight: 1.8 }}>
              PrintShop OS is a full e-commerce platform built specifically for print businesses. Customers configure, upload, and pay. Your production queue, AI prepress checks, and admin panel run behind it. Three languages. Zero custom code needed.
            </p>
