...
const STYLES = `
  ...
  .hw-ba-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1px; }
...
const FLOW = [
  { step: '01', label: 'Job comes in via WhatsApp → Logged immediately, no manual entry' },
  { step: '02', label: 'Photograph job sheet → AI creates full record in 10 seconds' },
  { step: '03', label: 'AI creates full record → 0 admin time spent' },
  { step: '04', label: 'Quote emailed to client → Sent in under 2 minutes' },
  { step: '05', label: 'Accepted — one-click invoice → No paperwork required' },
]
      ...
      <main style={{ background: '#080808' }}>
        ...
        <section style={{ background: '#0F0F0F', borderBottom: '1px solid #1A1A1A', padding: '3rem 2rem' }}>
          <div style={{ maxWidth: '72rem', margin: '0 auto' }}>
            ...
            <div style={{ display: 'grid', gap: '1px', background: '#1A1A1A' }} className="hw-grid-2">
              <div style={{ background: '#141414', padding: '28px 32px' }}>
                ...
                {['Jobs tracked on WhatsApp and paper', 'Quotes typed manually in Word', 'Time tracked on paper — disputes lost', 'XRechnung filed by accountant or skipped', 'Hours lost to admin per job'].map(item => (
              ...
              <div style={{ background: '#141414', padding: '28px 32px' }}>
                ...
                {['Job record created from photo in 10 seconds', 'Quote generated by AI in under 2 minutes', 'GPS time logs — every dispute winnable', 'XRechnung in one click — no accountant needed', 'Up to 60% less admin time per job'].map(item => (
