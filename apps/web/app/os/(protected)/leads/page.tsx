...
export default function LeadsPage() {
  ...
  const inpStyle: React.CSSProperties = { width: '100%', background: 'var(--brand-surface)', border: '1px solid var(--brand-border)', borderRadius: 'var(--radius-sm)', color: 'var(--brand-text)', fontFamily: sans, fontSize: 'var(--text-micro)', padding: '9px 12px', outline: 'none', boxSizing: 'border-box' }
    ...
    <div style={{ padding: '32px 40px' }}>
      ...
      {showNew && (
        <div style={{ position: 'fixed', inset: 0, background: 'color-mix(in srgb, var(--brand-text) 45%, transparent)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 'var(--space-5)' }}>
          <div style={{ background: 'var(--brand-surface-subtle)', border: '1px solid color-mix(in srgb, var(--brand-primary) 30%, transparent)', width: '100%', maxWidth: '440px', borderRadius: 'var(--radius-sm)', padding: '28px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
              <button type="button" onClick={createLead} disabled={creating} style={{ background: 'var(--brand-primary)', border: 'none', borderRadius: 'var(--radius-sm)', color: 'var(--brand-text)', fontFamily: sans, fontWeight: 700, fontSize: 'var(--text-micro)', padding: '10px 20px', cursor: creating ? 'wait' : 'pointer', opacity: creating ? 0.6 : 1 }}>
                {creating ? t.common.saving : t.leads.saveLead}
              </button>
              <button type="button" onClick={() => { setShowNew(false); setCreateErr('') }} style={{ background: 'none', border: '1px solid var(--brand-border)', borderRadius: 'var(--radius-sm)', color: 'var(--brand-text)', fontFamily: sans, fontSize: 'var(--text-micro)', padding: '10px 16px', cursor: 'pointer' }}>
                {t.common.cancel}
              </button>
            </div>
          </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--space-5)' }}>
        <div>
          <h1 style={{ fontFamily: sans, fontSize: '24px', fontWeight: 'var(--weight-heading)', color: 'var(--brand-text)', letterSpacing: '-0.02em', margin: '0 0 var(--space-1)' }}>{t.leads.heading}</h1>
        ...
        <button onClick={() => { setShowNew(true); setCreateErr('') }} style={{ background: 'var(--brand-primary)', border: 'none', borderRadius: 'var(--radius-sm)', color: 'var(--brand-text)', fontFamily: sans, fontWeight: 700, fontSize: 'var(--text-micro)', padding: '10px 18px', cursor: 'pointer' }}>
          {t.leads.newLead}
        </button>
      </div>

      {/* Filters */}
      <div style={{ display: 'flex', gap: 'var(--space-3)', marginBottom: '20px', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} style={{ padding: 'var(--space-5) var(--space-4)', fontFamily: sans, fontSize: 'var(--text-micro)', color: 'var(--brand-text-muted)' }}>{t.leads.empty}</td></tr>
            ) : (
              filtered.map(lead => (
                <tr key={lead.id} style={{ borderBottom: '1px solid var(--brand-border)', cursor: 'pointer' }} onClick={() => setSelected(lead)}>
                  ...
                  <td style={{ padding: 'var(--space-3) var(--space-4)' }}>
                    <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }} onClick={e => e.stopPropagation()}>
                      ...
                      <button onClick={() => deleteLead(lead.id)} style={{ fontFamily: mono, fontSize: 'var(--text-label-dense)', color: 'var(--brand-text-muted)', background: 'none', border: 'none', cursor: 'pointer', padding: '6px 2px', margin: '-6px -2px', letterSpacing: '0.04em' }}>
      ...
      {selected && (
        <div style={{ position: 'fixed', top: 0, right: 0, width: 'min(400px, 100vw)', height: '100vh', background: 'var(--brand-surface)', borderLeft: '1px solid var(--brand-border)', zIndex: 200, overflowY: 'auto' }}>
          ...
          <div style={{ padding: 'var(--space-5)' }}>
            <h2 style={{ fontFamily: sans, fontSize: '18px', fontWeight: 'var(--weight-heading)', color: 'var(--brand-text)', margin: '0 0 var(--space-1)' }}>{selected.name || t.leads.anonymous}</h2>
            {selected.company && <p style={{ fontFamily: mono, fontSize: 'var(--text-label)', color: 'var(--brand-text-muted)', margin: '0 0 var(--space-4)' }}>{selected.company}</p>}

            <StatusBadge status={selected.status} label={statusLabel(selected.status)} />

            <div style={{ marginTop: '20px', display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
              {[
            ...
            {selected.summary && (
              <div style={{ marginTop: 'var(--space-4)', background: 'var(--brand-surface-subtle)', border: '1px solid var(--brand-border)', borderLeft: '2px solid var(--brand-primary)', padding: '14px 16px' }}>
                <p style={{ fontFamily: mono, fontSize: 'var(--text-label-dense)', color: 'var(--brand-primary-text)', letterSpacing: '0.2em', textTransform: 'uppercase', margin: '0 0 var(--space-2)' }}>{t.leads.aiSummary}</p>
