    ...
    return (
      <main style={{ background: 'var(--brand-background)', minHeight: '100vh', padding: 'var(--section-y-feature) var(--section-x) var(--section-y)' }}>
        <div style={{ maxWidth: '960px', margin: '0 auto' }}>
          ...
          <h1 style={{ margin: '0 0 var(--space-4)' }}>
            {t(locale, 'Ausgewählte Arbeiten.', 'Selected work.')}
          </h1>
          <p style={{ ...sans, fontSize: 'var(--text-body)', color: 'var(--brand-text-secondary)', margin: '0 0 var(--space-8)', maxWidth: '640px', lineHeight: 1.7 }}>
            {t(locale,
          ...
          <div style={{ display: 'grid', gap: 'var(--space-4)' }}>
            {PORTFOLIO.map((item, i) => (
              <article key={i} className="card">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 'var(--space-3)', gap: '20px', flexWrap: 'wrap' }}>
                  <div>
                    <p style={{ ...mono, fontSize: 'var(--text-label)', color: 'var(--brand-text-secondary)', letterSpacing: '0.12em', textTransform: 'uppercase', margin: '0 0 6px' }}>{item.type}</p>
                    <h2 className="h-card" style={{ margin: '0 0 var(--space-1)' }}>{item.title}</h2>
                    <p style={{ ...sans, fontSize: '14px', color: 'var(--brand-text-secondary)', margin: 0 }}>{item.client}</p>
                  </div>
                </div>
                <p style={{ ...sans, fontSize: '16px', color: 'var(--brand-text-secondary)', margin: '0 0 var(--space-4)', lineHeight: 1.65 }}>{item.summary}</p>
                <div style={{ display: 'flex', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
                  {item.metrics.map(m => (
                    <span key={m} style={{ ...mono, fontSize: '12px', color: 'var(--brand-text-secondary)', background: 'color-mix(in srgb, var(--brand-primary) 8%, transparent)', border: '1px solid color-mix(in srgb, var(--brand-primary) 20%, transparent)', padding: '4px 10px', borderRadius: 'var(--radius-md)', letterSpacing: '0.03em' }}>
                      {m}
                    </span>
                  ))}
                </div>
  ...
  return (
    <main style={{ background: 'var(--brand-background)', minHeight: '100vh', padding: 'var(--section-y-feature) var(--section-x) var(--section-y)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <form onSubmit={submit} style={{ width: '100%', maxWidth: '420px' }}>
        ...
        <p style={{ ...sans, fontSize: 'var(--text-small)', color: 'var(--brand-text-secondary)', margin: '0 0 var(--space-6)', textAlign: 'center', lineHeight: 1.6 }}>
        ...
        />
        {error && (
          <p role="alert" style={{ ...mono, fontSize: 'var(--text-micro)', color: 'var(--semantic-danger)', margin: 'var(--space-3) 0 0' }}><Icon name="warning" size="xs" /> {error}</p>
        )}
        <button
          type="submit"
