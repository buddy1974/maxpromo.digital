...
import type { PolicyRecord } from "@/lib/mock/ai-governance";
import { getTranslations } from "next-intl/server";

export async function PolicyChecklist({ items }: { items: PolicyRecord[] }) {
  const t = await getTranslations("demo.policy");
  const g = await getTranslations("governancePage");
  const done = items.filter((i) => i.done).length;
  return (
    <div className="rounded-lg border border-hairline bg-surface p-5 shadow-sm">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-ink">{g("policyChecklist")}</h3>
